...
export default async function NewContractPage({
  ...
  const { error } = await requireRole(["brand"], "/app/brand/contracts/new");
  ...
  async function createContract(formData: FormData) {
    ...
    if (!brand) {
      redirect(
        "/app/brand/contracts/new?error=Missing%20brand%20profile.%20Update%20your%20settings."
      );
    }
    ...
    if (error) {
      ...
      redirect(
        `/app/brand/contracts/new?error=${encodeURIComponent(error.message)}`
      );
    }

    revalidatePath("/app/brand/contracts");
    redirect("/app/brand/contracts?created=1");
  }
