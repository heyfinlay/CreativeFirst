...
export default async function ContractDetailPage({
  ...
  async function updateApplicationStatus(formData: FormData) {
    ...
    if (error) {
      ...
      redirect(
        `/app/brand/contracts/${params.id}?error=${encodeURIComponent(
          error.message
        )}`
      );
    }

    revalidatePath(`/app/brand/contracts/${params.id}`);
  }
  ...
  const { data: applications } = await supabase
    .from("applications")
    .select("id, pitch, status, creator_user_id, profiles ( display_name )")
  ...
  return (
    <main className="flex w-full flex-col gap-8">
      <Link className="text-sm text-ink-700" href="/app/brand/contracts">
        ← Back to contracts
      </Link>
      ...
      <section className="rounded-3xl border border-white/60 bg-white/80 p-6 text-sm text-ink-700 shadow-soft">
        ...
        {typedApplications && typedApplications.length > 0 ? (
          ...
          <p className="mt-4 text-sm text-ink-700">No applicants yet.</p>
        )}
      </section>
    </main>
  );
